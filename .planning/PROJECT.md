...
| Carga manual de restaurantes por YAML reemplaza el onboarding web; frontend congelado como legacy | 2026-06-12: El wizard de onboarding (Phase 2) quedó con bugs (`slug_taken`) y es overhead innecesario para un piloto de 1 restaurante. Un script CLI (`pnpm cargar-restaurante`) que lee un archivo YAML y hace upsert directo con service role es más rápido, robusto y mantenible. El frontend existente queda congelado — ni se borra ni se mantiene activamente. Los datos del piloto Wonder se cargan con `restaurantes/wonder.yaml`. | ✅ Implementado (QUICK-M94) |
